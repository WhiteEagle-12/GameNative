...
public class WinHandler {
        ...
        switch (requestCode) {
            ...
            case RequestCodes.GET_GAMEPAD_STATE:
                ...
                if (externalController2 != null && externalController2.getDeviceId() != gamepadId) {
                    Log.w(TAG, "GET_GAMEPAD_STATE ignoring mismatched currentController, requested=" + gamepadId);
                    externalController2 = null;
                }
                final ExternalController stateController = externalController2;
                final boolean enabled3 = stateController != null || useVirtualGamepad2;
                addAction(() -> {
                    sendData.rewind();
                    ...
                    if (enabled3) {
                        ...
                        if (useVirtualGamepad2) {
                            ...
                            stateController.state.writeTo(this.sendData);
                        }
                    }
                    sendPacket(port);
                });
    ...
    public boolean onGenericMotionEvent(MotionEvent event) {
        ...
        ExternalController controller = getControllerForSlot(slot);
        Timber.tag("GNInputTrace").d(
                "WinHandler.onGenericMotionEvent deviceId=%d slot=%d source=0x%s controller=%s axes[x=%.3f,y=%.3f,z=%.3f,rz=%.3f,hatX=%.3f,hatY=%.3f,l=%.3f,r=%.3f]",
                event.getDeviceId(),
                slot,
                Integer.toHexString(event.getSource()),
                controller != null ? controller.getName() + "#" + controller.getDeviceId() : "null",
        ...
        if (controller != null && controller.updateStateFromMotionEvent(event)) {
            ...
            if (slot == 0) {
                Timber.tag("GNInputTrace").d("WinHandler.onGenericMotionEvent handled state=%s", summarizeGamepadState(controller.state));
                sendGamepadState();
            }
            return true;
        }
    ...
    public boolean onKeyEvent(KeyEvent event) {
        ...
        Timber.tag("GNInputTrace").d(
                "WinHandler.onKeyEvent action=%d keyCode=%d repeat=%d deviceId=%d slot=%d source=0x%s controller=%s",
                event.getAction(),
                event.getKeyCode(),
                ...
                slot,
                Integer.toHexString(event.getSource()),
                controller.getName() + "#" + controller.getDeviceId()
        );
        ...
        if (buffer != null) sendMemoryFileState(controller, buffer);
        if (handled && slot == 0) {
            Timber.tag("GNInputTrace").d("WinHandler.onKeyEvent handled state=%s", summarizeGamepadState(controller.state));
            sendGamepadState();
        }
        return handled;
    }
